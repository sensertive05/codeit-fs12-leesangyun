...
function updateEntry(id, updates) {
  ...
  const entry = diary.find(function (item) {
    return item.id === id;
  });
...
function deleteEntry(id) {
  ...
  const index = diary.findIndex(function (item) {
    return item.id === id;
  });
...
function renderStats() {
  ...
  totalEl.textContent = `전체 일기: ${stats.total}개`;
  ...
  for (const mood in stats.moods) {
    ...
    item.textContent = `${MOOD_EMOJIS[mood]} ${moodNames[mood]}: ${stats.moods[mood]}개`;
...
if (entriesToRender.length === 0) {
  ...
  if (listTitle) {
    listTitle.textContent = `일기 목록 (${entriesToRender.length})`;
  }